...
public class GridLineView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        if (showGrid) {
            ...
            int offsetX = Math.round(width / 16f);
            // Vertical lines
            for (int i = 0; i < numColumns; i++) {
                canvas.drawLine(width * i / numColumns + offsetX, 0,
                    width * i / numColumns + offsetX, height, paint);
            }
            // Horizontal lines
            for (int i = 0; i < numRows; i++) {
                canvas.drawLine(0, height * i / (numRows - 0.5f), width,
                    height * i / (numRows - 0.5f), paint);
            }
        }
    }
}